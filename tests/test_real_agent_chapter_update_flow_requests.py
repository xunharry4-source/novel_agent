...
def test_real_agent_chapter_update_flow_requests():
    chain = make_full_chain("agent_chapter_update")
    chapter_id = f"chapter_target_{chain['suffix']}"
    original_content = "原始章节：林澈在北港灯塔封锁离港信号后，开始核对潮汐刻度异常。"
    updated_name = f"Agent Chapter Updated {chain['suffix']}"
    payload = {
        ...
        "name": updated_name,
        "content": "修改草案：林澈在北港灯塔复核登记簿、追踪外海群岛异常航线，并在维持禁航信号的同时确认下一步调查目标。",
    }
    try:
        create_chapter(
            world_id=chain["world_id"],
        ...
        create_chapter(
            ...
            name=f"Agent Chapter Base {chain['suffix']}",
            content=original_content,
        )

        run = start_agent("chapter", "update", payload, "真实测试章节 Agent 修改流程")
        ...
        assert "保持世界、世界观、小说、大纲和章节承接一致" in modify_prompt, modify_prompt
        assert len(
            {
