...
WORKFLOW_DESCRIPTION = "新增章节内容总结工作流：输入章节正文 -> 生成下游摘要 -> 检查是否过度简化 -> 人工确认 -> 写入 downstream_summaries。"
WORKFLOW_STEPS = {
    ...
    "initial_expansion": {"step_index": 2, "step_title": "步骤 2：调用 LLM 简化章节正文", "function": "为新增章节正文生成下游摘要", "description": "调用章节内容总结专属 LLM，把章节正文压缩成下游摘要。"},
    "review": {"step_index": 3, "step_title": "步骤 3：检查摘要是否错误简化", "function": "检查是否丢失场景推进、动作因果和结果", "description": "如果摘要太短、没有抓住场景推进，或者根本没完成压缩，就不允许通过。"},
    "human": {"step_index": 4, "step_title": "步骤 4：人工确认", "function": "等待用户批准或要求重做", "description": "批准则写库；否则进入修改节点。"},
    "modify_content": {"step_index": 5, "step_title": "步骤 5：按意见重做章节正文摘要", "function": "根据人工意见返工 downstream_summary", "description": "只允许调整 downstream_summary，不得改动原始 content。"},
    "commit": {"step_index": 6, "step_title": "步骤 6：写入总结库", "function": "写入 downstream_summaries", "description": "把章节正文原文快照和最终摘要写入 downstream_summaries。"},
}
NODE_ANNOTATIONS = {
    "input": {"input_annotation": "输入应包含章节正文 content。", "output_annotation": "输出 accepted=true。", "next_step_annotation": "下一步进入章节正文摘要 LLM 节点。"},
    "initial_expansion": {"input_annotation": "输入是完整章节正文。", "output_annotation": "输出 downstream_summary。", "next_step_annotation": "下一步进入摘要质量检查节点。"},
...
def _check_summary_quality(source_text: Any, candidate_text: Any) -> List[str]:
    ...
    if source.count("\n") >= 8 and candidate.count("\n") <= 1:
        errors.append("摘要把多段场景压成单句，疑似错误简化。")
    return errors
...
3. payload.downstream_summary 输出章节正文下游摘要。
4. 保留所有业务 ID 和 name。
...
{{
  ...
  "payload": {{
    ...
    "name": "[保留输入 name]",
    "content": "[保留输入原始章节正文]",
    "downstream_summary": "[章节正文下游摘要]"
  }},
  "expanded_input": {{
    "content_seed": "[原始章节正文]",
    "summary_goal": "[这次摘要重点保留的场景推进]",
    "must_keep": ["[必须保留的信息]"]
  }},
  "expansion_notes": "[本次摘要保留了哪些动作与结果]"
}}
"""
