...
AGENT_NAME = "outline_summary_update_agent"
INITIAL_EXPANSION_AGENT_NAME = "outline_summary_update_llm"
MODIFY_CONTENT_AGENT_NAME = "outline_summary_update_modify_llm"
HUMAN_FEEDBACK_AGENT_NAME = "outline_summary_update_human_feedback_modify_llm"
ENTITY_TYPE = "outline_summary_update"
PRIMARY_FIELD = "summary"
SUMMARY_SCOPE = "outline"
SUMMARY_ACTION = "update"
WORKFLOW_DESCRIPTION = "修改分卷大纲总结工作流：输入原始分卷大纲全文 -> 调用独立 LLM 生成修改后的下游摘要 -> 检查是否过度简化 -> 人工确认 -> 写入 downstream_summaries。"
WORKFLOW_STEPS = {
    "input": {"step_index": 1, "step_title": "步骤 1：接收修改后的分卷大纲输入", "function": "记录待总结的大纲全文和目标实体", "description": "保留 target_id、outline_id、novel_id、world_id、worldview_id、name 和原始 summary。"},
    "initial_expansion": {"step_index": 2, "step_title": "步骤 2：调用 LLM 生成修改版摘要", "function": "为修改后的分卷大纲生成下游摘要", "description": "调用修改分卷大纲总结专属 LLM，生成新的下游摘要，不得覆盖原始 summary。"},
    "review": {"step_index": 3, "step_title": "步骤 3：检查摘要质量", "function": "检查是否过度简化、错误简化或没有真正简化", "description": "重点检查修改后的摘要是否仍保留卷级主冲突、关键转折和结局方向。"},
    "human": {"step_index": 4, "step_title": "步骤 4：人工确认", "function": "等待用户批准或要求重做", "description": "批准后写库；不同意则进入修改节点。"},
    "modify_content": {"step_index": 5, "step_title": "步骤 5：按意见重做修改版摘要", "function": "根据人工意见返工摘要", "description": "只允许调整 downstream_summary，不得改动 summary 原文和业务 ID。"},
    "commit": {"step_index": 6, "step_title": "步骤 6：写入总结库", "function": "写入 downstream_summaries", "description": "写入最新的修改版分卷大纲下游摘要。"},
}
NODE_ANNOTATIONS = {
    "input": {"input_annotation": "输入必须包含 target_id 或 outline_id，以及原始 summary。", "output_annotation": "输出 accepted=true。", "next_step_annotation": "下一步进入修改版分卷大纲摘要 LLM 节点。"},
    "initial_expansion": {"input_annotation": "输入是修改后的完整分卷大纲。", "output_annotation": "输出新的 downstream_summary。", "next_step_annotation": "下一步进入摘要质量检查节点。"},
    "review": {"input_annotation": "输入是原文和候选摘要。", "output_annotation": "输出 passed、errors、review_feedback。", "next_step_annotation": "通过则进入人工确认；失败则进入修改节点。"},
    "human": {"input_annotation": "输入是候选摘要和检查结果。", "output_annotation": "输出 decision、feedback、revision_mode。", "next_step_annotation": "批准则写库；要求修改则进入修改节点。"},
    "modify_content": {"input_annotation": "输入是原始大纲全文和人工修改意见。", "output_annotation": "输出返工后的 downstream_summary。", "next_step_annotation": "下一步回到检查节点。"},
    "commit": {"input_annotation": "输入是人工批准后的最终摘要。", "output_annotation": "输出 downstream_summaries 写库结果。", "next_step_annotation": "写库完成后结束。"},
}


class OutlineSummaryUpdateState(TypedDict, total=False):
    action: str
    message: str
...
def _check_summary_quality(source_text: Any, candidate_text: Any) -> List[str]:
    ...
    errors: List[str] = []
    if not candidate:
        return ["修改版下游摘要为空，属于错误简化。"]
    source_len = len(source)
    candidate_len = len(candidate)
    if source_len >= 600 and candidate_len < max(150, int(source_len * 0.13)):
        errors.append(f"摘要过短：原文 {source_len} 字，摘要只有 {candidate_len} 字。")
    if source_len >= 240 and candidate_len > int(source_len * 0.97):
        errors.append("摘要几乎没有压缩，未达到下游摘要目标。")
...
【摘要要求】
1. 保留卷名、卷级主矛盾、关键事件链、核心转折、高潮和结局方向。
2. 可以压缩细节，但不能压掉核心冲突推进。
3. 不允许把摘要写成小说正文。
4. 不允许改写 payload.summary 原文。
...
【输出要求】
1. 只返回合法 JSON。
2. payload.summary 保留原文。
3. payload.downstream_summary 写修改后的下游摘要。
...
{{
  ...
  "payload": {{
    ...
    "downstream_summary": "[修改版分卷大纲下游摘要]"
  }},
  "expanded_input": {{
    "summary_seed": "[原始分卷大纲全文]",
    "summary_goal": "[这次摘要最该保留的卷级信息]",
    "must_keep": ["[必须保留的信息]"]
  }},
  "expansion_notes": "[这版摘要保留了哪些重点]"
}}
"""
...
2. 不得改动 payload.summary 原文。
3. 必须修复“过短、丢主矛盾、丢关键转折、没有真正简化”等问题。
4. 仍然必须保持摘要短于原文。

【输出要求】
1. 只返回合法 JSON。
2. payload.summary 保留原文。
3. payload.downstream_summary 返回返工后的摘要。
4. 保留所有业务 ID 和 name。
...
def input_node(state: OutlineSummaryUpdateState) -> OutlineSummaryUpdateState:
...
def initial_expansion_node(state: OutlineSummaryUpdateState) -> OutlineSummaryUpdateState:
...
def review_node(state: OutlineSummaryUpdateState) -> OutlineSummaryUpdateState:
...
def modify_content_node(state: OutlineSummaryUpdateState) -> OutlineSummaryUpdateState:
    payload = dict(state.get("pending_payload") or state.get("payload") or {})
...
def human_node(state: OutlineSummaryUpdateState) -> OutlineSummaryUpdateState:
...
def route_after_human(state: OutlineSummaryUpdateState) -> str:
...
def commit_node(state: OutlineSummaryUpdateState) -> OutlineSummaryUpdateState:
    db = get_mongodb_db()
    payload = dict(state.get("pending_payload") or {})
