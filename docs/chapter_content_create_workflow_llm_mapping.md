# 创建章节内容工作流节点与 LLM 对应关系表

## 范围

- 工作流类型：`chapter`
- 业务动作：`create`
- 页面入口：`/workflow/chapter?action=create&chapter_outline_id=<父级章节大纲ID>`
...
- 当前“创建章节内容”工作流不存在节点 LLM 混用问题。
- “章节内容创建”本质上仍然走 `chapter/create` 工作流，只是在 payload 中额外带 `chapter_outline_id`，把新建正文挂到具体章节大纲下面。
- 本轮额外修复了一个真实产品缺陷：`chapter_outline_id` 曾在 LLM 节点解析后丢失，导致章节内容写库时失去父级章节大纲绑定。现在 [src/agents/chapter_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/chapter_agent.py) 会在 `initial_expansion` 和 `modify_content` 后强制继承 `chapter_outline_id`。
...
| --- | --- | --- | --- | --- | --- |
| `input` | 否 | `N/A` | `N/A` | [src/agents/chapter_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/chapter_agent.py) | 只接收 `chapter_outline_id`、父级上下文和正文输入，禁止调用 LLM |
| `initial_expansion` | 是 | `chapter_agent_initial_expansion` | `AGENT_MODELS.chapter_agent_initial_expansion` | [src/agents/chapter_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/chapter_agent.py) | 创建章节内容时的首次整理与扩充节点 |
...
| `commit` | 否 | `N/A` | `N/A` | [src/agents/chapter_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/chapter_agent.py) | 只负责真实写入 `prose.chapter_outline_id` 子记录，禁止调用 LLM |
...
- `NovelChapterContentManagement.tsx` 的“工作流添加章节内容”把 `chapter_outline_id` 填进 `/workflow/chapter?...`
...
- `chapter_outline_id` 只决定这条正文挂在哪个父级章节大纲下，不会引入新的共享 LLM reviewer。
- 如果未来再给 `chapter/create` 新增章节内容专属字段，不能只靠 Prompt 约束，必须像 `chapter_outline_id` 一样在代码层强制继承业务父级 ID。
