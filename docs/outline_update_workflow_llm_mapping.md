...
- 路由入口：`POST /api/hierarchy-agent/start` with `agent_type=outline` and `action=update`

## 结论

- 当前“修改分卷大纲”工作流不存在节点 LLM 混用问题。
...
| --- | --- | --- | --- | --- | --- |
| `input` | 否 | `N/A` | `N/A` | [src/agents/outline_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/outline_agent.py) | 只接收 `target_id`、`novel_id`、`world_id`、`worldview_id`、消息和 payload，禁止调用 LLM |
| `initial_expansion` | 是 | `outline_agent_initial_expansion` | `AGENT_MODELS.outline_agent_initial_expansion` | [src/agents/outline_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/outline_agent.py) | 修改分卷大纲时的首次整理与扩充节点 |
...
| `commit` | 否 | `N/A` | `N/A` | [src/agents/outline_agent.py](/Users/harry/Documents/git/novel_agent/src/agents/outline_agent.py) | 只负责按 `target_id` 更新 `outlines` 集合，禁止调用 LLM |
